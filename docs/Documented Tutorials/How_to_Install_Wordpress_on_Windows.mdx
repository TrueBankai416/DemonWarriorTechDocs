...
```cmd
REM Get latest NSSM version dynamically by scraping download page
echo Getting latest NSSM version...
...
echo Downloaded %NSSM_VERSION% to %TEMP%\%NSSM_VERSION%.zip
```
...
dir "%TEMP%\nssm-*.zip" 2>nul && echo ✅ NSSM downloaded || echo ❌ NSSM missing
dir "%TEMP%\caddy_windows_amd64.zip" 2>nul && echo ✅ Caddy downloaded || echo ❌ Caddy missing
```

**All-in-One Download Script (Fully Dynamic):**
```cmd
@echo off
...
echo All downloads completed with latest versions!
