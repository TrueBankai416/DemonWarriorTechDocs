...
#### 8. **🔄 External Load Balancer (Optional)**
- **Purpose**: Additional load balancing if not using Caddy's built-in capabilities
- **Note**: Caddy includes built-in load balancing, health checks, and failover
- **Use Case**: Complex multi-tier architectures or legacy infrastructure
- **Alternatives**: Use Caddy's native `reverse_proxy` with multiple upstreams

#### 9. **🎯 Backend Service**
- **Applications**: Web apps, APIs, microservices
- **Protocols**: HTTP, gRPC, WebSocket (TCP/UDP requires caddy-l4 plugin)
...
```caddyfile
api.example.com {
    # Note: rate_limit requires github.com/mholt/caddy-ratelimit plugin
...
### **Rate Limiting**
Protect your services from abuse and DDoS attacks (requires the `github.com/mholt/caddy-ratelimit` plugin):

```caddyfile
example.com {
    # Requires github.com/mholt/caddy-ratelimit plugin from https://caddyserver.com/download
    rate_limit {
        zone example_zone
        key {remote_host}
        events 50
...
### **📚 Installation Guides**
- **[Installing Caddy on Linux](/Documented%20Tutorials/Caddy/Linux/Installing_Caddy_on_Linux)**: Complete Ubuntu/Debian/CentOS setup
- **[Installing Caddy on Windows](/Documented%20Tutorials/Caddy/Windows/Installing_Caddy_on_Windows)**: Windows Server and desktop installation
