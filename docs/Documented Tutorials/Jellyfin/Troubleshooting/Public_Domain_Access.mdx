...
```powershell
Invoke-WebRequest -Uri https://jellyfin.yourdomain.com -Method Head
...
- Check Jellyfin is listening on correct port: `netstat -an | findstr 8096`
...
**Check Caddy Logs**:
```powershell
# View Caddy logs in Event Viewer
eventvwr.msc
# Navigate to Windows Logs → Application → Filter by "Caddy"

# Or check Caddy log files if configured
Get-Content "C:\caddy\logs\caddy.log" -Tail 50 -Wait
```

**Check Jellyfin Logs**:
```powershell
# View Jellyfin logs in Event Viewer
eventvwr.msc
# Navigate to Windows Logs → Application → Filter by "Jellyfin"

# Or check Jellyfin log files
Get-Content "$env:PROGRAMDATA\Jellyfin\Server\logs\jellyfin*.log" -Tail 50
```
