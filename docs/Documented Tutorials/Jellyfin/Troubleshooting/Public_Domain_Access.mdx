...
1. **Go to [portchecker.co](https://portchecker.co/)**
...
**After making firewall changes, retest ports at [portchecker.co](https://portchecker.co/) before continuing.**
...
**Solutions**:
- Verify Jellyfin is running: `sudo systemctl status jellyfin`
- Check Jellyfin is listening on correct port: `netstat -tlnp | grep 8096`
...
- **Leave empty** for most setups
- **OR** set to `/` if required
...
1. **Clear the Base URL field** (leave it empty)
2. **Click "Save"**
