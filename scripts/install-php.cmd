...
powershell -NoProfile -Command "try { $service = Get-Service -Name 'PHP-CGI' -ErrorAction SilentlyContinue; if ($service) { Write-Host '✅ PHP-CGI service already exists'; Set-Service -Name 'PHP-CGI' -StartupType Automatic; Write-Host '✅ PHP service set to automatic startup'; try { Start-Service -Name 'PHP-CGI' -ErrorAction SilentlyContinue; Write-Host '✅ PHP service started'; } catch { Write-Host '⚠️  Service start failed: Already running or permission issue'; } } else { Write-Host 'Creating PHP-CGI service...'; New-Service -Name 'PHP-CGI' -BinaryPathName 'C:\Tools\PHP\php-cgi.exe -b 127.0.0.1:9000' -DisplayName 'PHP FastCGI Service' -Description 'PHP FastCGI service for web applications' -StartupType Automatic; Write-Host '✅ PHP-CGI service created successfully'; try { Start-Service -Name 'PHP-CGI'; Write-Host '✅ PHP service started'; } catch { Write-Host '⚠️  Service created but failed to start: $_'; } } } catch { Write-Host '❌ Service operation failed (requires admin): $_'; Write-Host 'Please run as administrator to create PHP service'; }"
